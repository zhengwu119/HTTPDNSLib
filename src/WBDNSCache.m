...
- (instancetype)init {
    if (self = [super init]) {
        ...
        _testSpeedTimer = [WBDNSWeakTimer scheduledTimerWithTimeInterval:10 target:self selector:@selector(processPeriodicTestTask) userInfo:nil repeats:YES];
        [self processPeriodicTestTask];
    }
    return self;
}
...
- (NSArray *)getDomainServerIpFromURL:(NSString *)urlString {
    if (!_dnsCacheConfig.config.enableHttpDnsCache || !connect) {
        return nil;
    }
...
- (void)processPeriodicTestTask {
    ...
    dispatch_async(queue, ^{
        WBDNSTCPSpeedTester *_speedTester = [[WBDNSTCPSpeedTester alloc]init];
        int sec = [_speedTester testSpeedOf:@"119.29.29.29"];
        connect = !(sec == WBDNS_SOCKET_CONNECT_TIMEOUT_RTT);
//        NSLog(@"network connected1:%@,TIME:%@",@(connect),@(sec));
        if (!connect) {
            int sec2 = [_speedTester testSpeedOf:@"114.114.115.115"];
            connect = !(sec2 == WBDNS_SOCKET_CONNECT_TIMEOUT_RTT);
            NSLog(@"network connected2:%@,TIME:%@",@(connect),@(sec2));
        }
    });
}
